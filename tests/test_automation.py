...
def test_the_window_cannot_sit_inside_the_capture_region():
    ...
    code = (
        ...
        "def set_region(rl, rt, rw, rh):\n"
        "    app.x_var.set(str(rl)); app.y_var.set(str(rt))\n"
        "    app.w_var.set(str(rw)); app.h_var.set(str(rh))\n"
        "def overlaps(rl, rt, rw, rh):\n"
        "    wx, wy = root.winfo_x(), root.winfo_y()\n"
        "    ww, wh = root.winfo_width(), root.winfo_height()\n"
        "    return not (wx + ww <= rl or wx >= rl + rw or wy + wh <= rt or wy >= rt + rh)\n"
        ""
        "# Screen size is FAKED for every case below, not left as whatever the\n"
        "# real host happens to have - the whole point of this test is a bug\n"
        "# that only a SMALL screen (a real CI virtual display, not a normal\n"
        "# monitor) could trigger, so leaving it to the real host would make\n"
        "# this test's outcome depend on whatever machine happens to run it.\n"
        "# 螢幕尺寸在下面每個案例都是假造的，不是交給真實主機決定——這個測試\n"
        "# 要守的 bug，本來就只有「小」螢幕（真實 CI 的虛擬畫面，不是一般\n"
        "# 螢幕）才會踩到，如果交給真實主機，這個測試的結果會隨著誰來執行\n"
        "# 而不一樣。\n"
        ""
        "# 1a) Plenty of room on the right - the original, unchanged happy\n"
        "# path: stays visible, moves beside the region.\n"
        "region_1a = (200, 200, 640, 700)\n"
        "set_region(*region_1a)\n"
        ...
        "root.update_idletasks()\n"
        "assert root.state() != 'iconic', 'must not iconify when the right side clearly fits / 右邊明明放得下卻被最小化了'\n"
        "assert not overlaps(*region_1a), f'still overlaps after moving right / 往右移仍然重疊: ({root.winfo_x()},{root.winfo_y()})'\n"
        "print('FITS_RIGHT_OK')\n"
        ""
        "# 1b) Region sits far enough from the screen's left edge to leave\n"
        "# room there, but the screen is too small for right-of or below-the-\n"
        "# region to fit - exercises the branch added by this fix (the old\n"
        "# code only ever checked right and below, so 'fits on the left\n"
        "# instead' was never handled). region_left(500) >= window width(400)\n"
        "# is what makes the left side the one that actually has room.\n"
        "root.deiconify(); app._minimized_for_wait = False\n"
        "region_1b = (500, 50, 100, 100)\n"
        "set_region(*region_1b)\n"
        "root.winfo_screenwidth = lambda: 900; root.winfo_screenheight = lambda: 600\n"
        "# Start the window ON the region (not the leftover +250+250 from 1a,\n"
        "# which does not even overlap this different, smaller region) so\n"
        "# there is genuinely something for _keep_window_clear_of_capture to\n"
        "# fix, exactly as a real overlap would look.\n"
        "root.geometry(f'400x470+{region_1b[0]}+{region_1b[1]}')\n"
        "root.update_idletasks()\n"
        "app._keep_window_clear_of_capture()\n"
        "root.update_idletasks()\n"
        "assert root.state() != 'iconic', 'must not iconify when the left side fits / 左邊明明放得下卻被最小化了'\n"
        "assert not overlaps(*region_1b), f'still overlaps after the left-side fallback / 靠左備援後仍然重疊: ({root.winfo_x()},{root.winfo_y()})'\n"
        "# Check the EXACT expected position, not just 'somewhere non-\n"
        "# overlapping' - the old code's blind (0,0) fallback would ALSO\n"
        "# happen to not overlap this particular region (it is far enough to\n"
        "# the right that x=0 misses it on the x-axis alone), so a bare\n"
        "# overlap check cannot tell 'used the new left-of-region branch'\n"
        "# apart from 'got lucky with the old unconditional (0,0)'. The exact\n"
        "# coordinate can: this fix's left branch always lands at\n"
        "# (region_left - window_width - 10, region_top), never (0, 0).\n"
        "expected_x = region_1b[0] - root.winfo_width() - 10\n"
        "assert (root.winfo_x(), root.winfo_y()) == (expected_x, region_1b[1]), (\n"
        "    f'did not land at the expected left-of-region position / '\n"
        "    f'沒有落在預期的靠左位置: got ({root.winfo_x()},{root.winfo_y()}), '\n"
        "    f'expected ({expected_x},{region_1b[1]})')\n"
        "print('FITS_LEFT_OK')\n"
        ""
        "# 1c) THE BUG: screen too small to fit the window beside the region\n"
        "# on ANY of the four sides (this is the exact real 1024x768 CI\n"
        "# failure - region (200,200,640,700), a 400x470 window). The old\n"
        "# code silently placed the window at an unchecked (0,0), which still\n"
        "# overlapped. Must now iconify instead of lying about being clear.\n"
        "root.deiconify(); app._minimized_for_wait = False\n"
        "set_region(200, 200, 640, 700)\n"
        "root.winfo_screenwidth = lambda: 1024; root.winfo_screenheight = lambda: 768\n"
        "root.geometry('400x470+250+250')\n"
        "root.update_idletasks()\n"
        "app._keep_window_clear_of_capture()\n"
        "root.update()\n"
        "assert root.state() == 'iconic', 'must iconify when no side fits, not silently overlap / 四邊都放不下時應該最小化，不能悄悄重疊'\n"
        "print('ICONIFY_WHEN_NO_SIDE_FITS_OK')\n"
        ""
