...
- **`test_compat.py`** —— 驗證「宣稱支援 Python 3.9」是真的。逐檔在最低版本下
  ...
  這是在 1.0.0 帶著一個在 3.9 上會當掉的寫法出門之後才加的。
- **`test_solvers.py`** —— 用手工編出來的題目測求解邏輯與唯一性守門。每個測試都用
  **獨立於求解器之外**的方式重新檢查規則，求解器就算寫錯也不可能靠自我驗證通過。
- **`test_digits.py`** —— 驗證一個數字要嘛讀對、要嘛回報成讀不出來，
  絕不會默默讀成另一個數字。用十種字型、以及完全沒有系統字型的情況下檢查。
- **`test_recognition.py`** —— 真實截圖。每一張圖都是真的發生過的 bug：
  被誤判成 Patches 的淡色系 Queens、整個偏一欄的 Tango 網格、
  白色縫隙被當成數字筆畫的虛線 Patches 標籤。
- **`test_zip_dots.py`** —— 圓點偵測、內孔抽取，以及「讀不出來就拒絕猜測」。
  真值是人工看著圖讀出來的。
...
- **`test_settings.py`** —— 設定檔裡不合法的擷取範圍會被丟掉、不會傳到
  GUI 手上讓它在啟動時當掉。
- **`test_image_io.py`** —— 讀寫圖片一律回傳值，絕不拋例外：資料夾、
  鎖住的檔案、寫不進去的路徑都要明確失敗。
- **`test_cli.py`** —— 命令列的 `--region` 解析：格式錯誤或寬高不是正數
  要有清楚的錯誤訊息，不能是原始的例外。
...
- **檢查補點只涵蓋 Tango、Queens、Sudoku。** Zip 與 Patches 是用拖曳填的，
  結果不會被讀回來。
- **填答途中棋盤「移動」偵測不到。** 執行時請不要捲動頁面或移動視窗 ——
  剩下的每一次點擊都會偏一格。
- **命令列 `--go` 沒有盤面保護。** 圖形介面在棋盤被換掉時會停，命令列不會。
- **填答途中關閉視窗可能讓滑鼠鍵卡在按下狀態。** 請先按「停止」。
- **圖形介面即使只用圖片模式也需要安裝 `mss`**，而且設定檔損壞會讓它完全開不起來。
- **螢幕模式僅限 Windows，而且假設棋盤在主螢幕上。**
- **圖片模式的門檻值是照 iPhone 13 Pro 的截圖校準的。**
- **沒有任何測試素材含有 0 或 7。** 這兩個數字範本來自字型，從未用真實盤面驗證過。
- **辨識與求解的錯誤訊息是固定的中英文串接字串**，因此不受語言設定影響。
